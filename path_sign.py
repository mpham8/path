#!/usr/bin/env python3
"""
PATH arrival sign for a 1024x600 IPS HDMI panel driven straight to the
...
# CONFIG -- edit these
# ---------------------------------------------------------------------------
WIDTH, HEIGHT = 1024, 600          # match your panel's native resolution
FB_DEVICE = "/dev/fb0"
...
def _draw_arrival_time(draw, right_x, cy, minutes, num_px, unit_px):
    """Draw '13 min' with the number large and ' min' in regular weight."""
    if minutes is None:
        font = _font(FONT_REGULAR_CANDIDATES, num_px)
        bbox = draw.textbbox((0, 0), "--", font=font)
        x = right_x - (bbox[2] - bbox[0])
        draw.text((x, cy - (bbox[3] - bbox[1]) / 2 - bbox[1]), "--", font=font, fill=TEXT)
        return
    ...
    total_w = (num_bbox[2] - num_bbox[0]) + (unit_bbox[2] - unit_bbox[0])
    x = right_x - total_w

    num_y = cy - (num_bbox[3] - num_bbox[1]) / 2 - num_bbox[1]
    draw.text((x, num_y), num_str, font=num_font, fill=TEXT)

    unit_x = x + num_bbox[2] - num_bbox[0]
    unit_y = num_y + num_bbox[3] - unit_bbox[3]
    draw.text((unit_x, unit_y), unit_str, font=unit_font, fill=TEXT)


def _draw_footer(d, footer_y, footer_h):
    d.rectangle([0, footer_y, WIDTH, HEIGHT], fill=FOOTER_BG)

    chevron_w = int(WIDTH * 0.24)
    tip_x = chevron_w + int(footer_h * 0.32)
    ...
    tbbox = d.textbbox((0, 0), time_str, font=time_font)
    tx = int(chevron_w * 0.18)
    ty = mid_y - (tbbox[3] - tbbox[1]) / 2 - tbbox[1]
    d.text((tx, ty), time_str, font=time_font, fill=FOOTER_TEXT)
...
def render_route_page(route, trains, line_color):
    ...
    num_px = min(int(row_h * 0.55), 150)
    unit_px = min(int(row_h * 0.40), 96)
    dot_color = hex_to_rgb(line_color)
    dot_r = int(num_px * 0.45)
    ...
    for i, (train, minutes) in enumerate(rows):
        ...
        l_h = lbbox[3] - lbbox[1]
        d.text((label_x, cy - l_h / 2 - lbbox[1]), label, font=label_font, fill=TEXT)

        _draw_arrival_time(d, time_x, cy, minutes, num_px, unit_px)
    ...
    try:
        while True:
            ...
            if preview:
                ...
                fname = f"path_sign_{STATION_CODE}_{slug}.png"
                img.save(fname)
                print(f"Wrote {fname}")
                route_idx += 1
